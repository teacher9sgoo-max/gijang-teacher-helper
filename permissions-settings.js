...
(function(){
  ...
  function permissionFor(key){
    ...
    return currentRole==='admin'||!!(account&&account.permissions&&account.permissions[key]);
  }
  ...
  function injectSettings(){
    ...
    if(nav&&!nav.querySelector('[data-tab="settings"]')){
      const b=document.createElement('button');b.dataset.tab='settings';b.textContent='내 설정';
      nav.insertBefore(b,members||null);
      b.addEventListener('click',function(){document.querySelectorAll('nav.tabs button').forEach(function(x){x.classList.remove('active');});b.classList.add('active');document.querySelectorAll('section.tab').forEach(function(x){x.classList.remove('active');});document.getElementById('tab-settings').classList.add('active');renderSettings();});
    }
  ...
  function updateDashboardEditControls(){
    const calendar=permissionFor('calendar'), notice=permissionFor('notice');
    const calToggle=document.getElementById('qa-cal-toggle'),noticeToggle=document.getElementById('qa-notice-toggle');
  ...
  function renderSettings(){
    ...
    renderDutySettings();if(currentRole==='admin')renderMasterPermissions();
  }
  ...
  function renderDutySettings(){
    const root=document.getElementById('duty-settings-area');if(!root)return;
    if(!permissionFor('duty')){root.innerHTML='<article class="card" style="margin-top:18px"><h3>조별 반복 급식 당번표</h3><p class="progress-note">급식 당번표 수정 권한이 없습니다. 마스터 관리자에게 권한을 요청하세요.</p></article>';return;}
    root.innerHTML='<article class="card" style="margin-top:18px"><div class="section-title" style="margin-bottom:12px"><div><h3>조별 반복 급식 당번표 수정</h3><p>기존 `조별 반복 당번표` 시트에 바로 저장됩니다.</p></div><div><label>수정 조 <select id="duty-edit-group"><option value="1">1조</option><option value="2">2조</option></select></label></div></div><div class="duty-edit-scroll"><table class="duty-edit-table"><thead><tr><th>업무</th><th>월</th><th>화</th><th>수</th><th>목</th><th>금</th></tr></thead><tbody id="duty-edit-body"></tbody></table></div><button class="btn primary" id="duty-edit-save" type="button" style="margin-top:14px">선택한 조 당번표 저장</button><div id="duty-edit-status" class="settings-message"></div></article>';
    const select=document.getElementById('duty-edit-group');select.value=String(weeklyDutyRotation.activeGroup||1);select.onchange=renderDutyEditRows;document.getElementById('duty-edit-save').onclick=saveDutyEdit;renderDutyEditRows();
  }
  ...
  establishLogin_=function(account,pw){originalEstablishLogin(account,pw);setTimeout(function(){renderSettings();showAlerts();},250);};
  ...
  loadAll=async function(){const result=await originalLoadAll();if(currentAccountId){renderSettings();showAlerts();}return result;};
  document.addEventListener('DOMContentLoaded',function(){
    ...
    if(originalButton){
      ...
      button.textContent='웹에서 수정';
      button.addEventListener('click',function(){
        const tab=document.querySelector('nav.tabs button[data-tab="settings"]');
        if(tab)tab.click();
        setTimeout(function(){document.getElementById('duty-settings-area')?.scrollIntoView({behavior:'smooth',block:'start'});},80);
      });
    }
    updateDashboardEditControls();
  });
