...
(function(){
  ...
  function renderMasterPermissions(){
    ...
    root.querySelectorAll('[data-save-perm]').forEach(function(button){button.onclick=async function(){const tr=button.closest('tr'),status=document.getElementById('permissions-status'),permissions={};tr.querySelectorAll('[data-perm]').forEach(function(x){permissions[x.dataset.perm]=x.checked;});button.disabled=true;status.className='settings-message';status.textContent='권한을 저장 중입니다...';const result=await postApp({action:'permissionUpdate',targetId:tr.dataset.permId,permissions:permissions,loginId:currentAccountId,password:currentLoginPassword});button.disabled=false;if(result.success){status.className='settings-message ok';status.textContent='권한을 저장했습니다. 해당 교사가 다음 로그인 또는 새로고침 후 적용됩니다.';setTimeout(function(){alert('완료되었습니다.\n권한을 저장했습니다. 해당 교사가 다음 로그인 또는 새로고침 후 적용됩니다.');},50);}else{status.className='settings-message err';status.textContent=result.error||'권한을 저장하지 못했습니다.';setTimeout(function(){alert('실패하였습니다.\n'+(result.error||'권한을 저장하지 못했습니다.'));},50);}};});
  }
  function showAlerts(){
